...
import { AriaToggleButton, AriaSwitch, AriaCheckbox } from '@/components/ui/aria-tab-button';
...
export function MarketplaceFilters({ filters, onChange, onApply, onReset }: Props) {
  ...
  const scorePct = (filters.minNeuronScore / 1000) * 100;
    ...
    <aside className="space-y-6" aria-label="Marketplace filters" data-testid="marketplace-filters">
        ...
        <div className="space-y-2">
          {AI_MODELS.map((m) => (
            <label key={m} className="flex items-center gap-2.5 cursor-pointer group">
              <AriaCheckbox
                checked={filters.aiModels.includes(m)}
      ...
      <div>
        ...
        <label className="flex items-center gap-3 cursor-pointer group">
          ...
          >
            <div className={cn('absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 pointer-events-none', filters.minRating >= 4 ? 'translate-x-5' : 'translate-x-0.5')} />
          </AriaSwitch>
      ...
      <div>
        ...
        <div className="relative h-6 flex items-center">
          ...
          <div
            className={cn('absolute left-0 h-1.5 rounded-full bg-accent-cyan', wPctClass(scorePct))}
            aria-hidden="true"
          />
          <input
            type="range"
      ...
      <label className="flex items-center gap-3 cursor-pointer group">
        ...
        >
          <div className={cn('absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 pointer-events-none', filters.tryBeforeBuy ? 'translate-x-5' : 'translate-x-0.5')} />
        </AriaSwitch>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2" role="group" aria-label="Pricing model filter">
          {PRICING_MODELS.map((pm) => (
            <AriaToggleButton
              key={pm.value}
              pressed={filters.pricingModels.includes(pm.value)}
