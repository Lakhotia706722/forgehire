...
export function MiniGateModal({ open, onClose, onPass, requiredScore }: MiniGateModalProps) {
  ...
  const [questions, setQuestions] = React.useState(MINI_GATE_QUESTIONS);
  ...
  return (
    <Modal
      ...
      title={phase === 'intro' ? 'Mini-Gate Test' : undefined}
      size="lg"
    >
      {phase === 'intro' && (
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-3">
            <Button
              size="md"
              className="flex-1"
              onClick={() => { setPhase('test'); setSecondsLeft(MINI_GATE_SECONDS); }}
        ...
        <div className="flex flex-col" style={{ minHeight: 480 }}>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-[rgba(255,255,255,0.06)]">
            ...
            {currentQ === questions.length - 1 ? (
              <Button size="sm" onClick={handleSubmit} data-testid="mini-gate-submit-btn">
                Submit
              </Button>
            ) : (
              <button
