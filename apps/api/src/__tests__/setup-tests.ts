...
// Test environment
process.env.DATABASE_URL = process.env.TEST_DATABASE_URL || 'postgresql://test:test@localhost:5432/neuronhire_test';
...
export const createTestEngineerProfile = async (userId: string, data?: Partial<{
  fullName: string;
  bio: string;
}>) => {
...
export const createTestCompanyProfile = async (userId: string, data?: Partial<{
  companyName: string;
  description: string;
  industry: string;
}>) => {
...
export const createTestContract = async (data: {
  ...
  const engineerProfileId = engineerUser?.engineerProfile?.id
