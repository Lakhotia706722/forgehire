...
describe('SearchService', () => {
  ...
  describe('searchEngineers', () => {
    it('should search with skill filter', async () => {
      ...
      mockTypesense.collections().documents().search.mockResolvedValue(mockResults);
      ...
      expect(mockTypesense.collections).toHaveBeenCalledWith('engineer_profiles');
    });
    ...
    it('should filter by NeuronScore range', async () => {
      ...
      const searchCall = mockTypesense.collections().documents().search.mock.calls[0][0];
    ...
    it('should filter by availability status', async () => {
      ...
      const searchCall = mockTypesense.collections().documents().search.mock.calls[0][0];
      expect(searchCall.filter_by).toContain('availabilityStatus:=available_now');
    });
