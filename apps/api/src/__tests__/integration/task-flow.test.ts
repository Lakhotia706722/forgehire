...
describeOrSkip("Task Flow Integration Test", () => {
  ...
  afterAll(async () => {
    ...
    await prisma.user
      .delete({ where: { id: engineerUser.id } })
      .catch(() => {});
  ...
  it("should complete full task lifecycle: create → enrich → escrow → publish → participate → submit → select winner → payout", async () => {
    ...
    task = await taskService.createTask(companyUser.id, {
      ...
      problemStatement:
        "We need an AI chatbot that can handle customer support queries with high accuracy. The chatbot should understand context, handle multiple languages, and integrate with our existing CRM system.",
      currentState:
        "Currently using manual support team, response time is 2-3 hours",
      expectedOutcome:
        "A production-ready chatbot with 90%+ accuracy, <1s response time, and seamless CRM integration",
      deliverables: [
        {
    ...
    const escrowOrder = await taskService.createEscrowOrder(
      task.id,
      companyUser.id,
    );
    ...
    const taskAfterEscrow = await prisma.task.findUnique({
      where: { id: task.id },
    });
    ...
    const publishedTask = await taskService.depositEscrow(
      task.id,
      companyUser.id,
      {
        taskId: task.id,
    ...
    const participation = await taskService.participateInTask(
      ...
      {
        approach:
          "I will use GPT-based model fine-tuned on your support data. Architecture: FastAPI backend with Redis caching, Docker containerized deployment. I have 5+ years experience building production chatbots.",
        estimatedTime: 25,
        proposedRate: null,
      },
    );
    ...
    const taskAfterParticipation = await prisma.task.findUnique({
      where: { id: task.id },
    });
    ...
    const submission = await taskService.submitTask(task.id, engineerUser.id, {
      description:
        "Completed chatbot with 92% accuracy on test dataset. API response time averages 0.8s. Fully containerized with Docker. Includes comprehensive documentation and deployment scripts.",
    ...
    const taskAfterSubmission = await prisma.task.findUnique({
      where: { id: task.id },
    });
    ...
    const evaluatedSubmission = await taskService.evaluateSubmission(
      ...
      companyUser.id,
      {
        ...
        score: 92,
        feedback:
          "Excellent work! Model accuracy exceeds requirements, API is fast and well-documented. Code quality is professional.",
        criteriaScores: {
          "Model Accuracy": 95,
          "Response Time": 90,
          "Code Quality": 90,
