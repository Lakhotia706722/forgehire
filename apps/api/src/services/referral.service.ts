...
export class ReferralService {
  ...
  async getReferralStats(userId: string) {
    ...
    const totalPurchases = referralLinks.reduce((sum, link) => sum + link.purchaseCount, 0);
    const totalCommission = referralLinks.reduce((sum, link) => sum + Number(link.totalCommission), 0);
