...
export class AssessmentGeneratorService {
  ...
  async generateAssessment(
    ...
    const questions = await this.selectMCQQuestions(
      skills,
      experienceLevel,
      30,
    );
    ...
    const caseScenario = await this.generateCaseScenario(
      skills,
      experienceLevel,
    );
  ...
  private async selectMCQQuestions(
    ...
    const difficultyDistribution =
      this.getDifficultyDistribution(experienceLevel);
    ...
    for (const [difficulty, percentage] of Object.entries(
      ...
      const selectedQuestions = await collection
        .aggregate([{ $match: query }, { $sample: { size: numQuestions } }])
        .toArray();
...
Return as JSON:
{
    ...
    return [
      {
        ...
        description:
          "Implement a data preprocessing function that handles missing values, normalizes features, and encodes categorical variables.",
  ...
  private getFallbackCaseScenario(): CaseScenario {
    return {
      ...
      scenario:
        "You are tasked with building a recommendation system for an e-commerce platform...",
      questions: [
