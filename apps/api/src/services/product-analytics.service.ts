...
export class ProductAnalyticsService {
  ...
  async getProductAnalytics(
    ...
    const refundRate =
      totalPurchases > 0 ? (totalRefunds / totalPurchases) * 100 : 0;
  ...
  async getEngineerDashboard(engineerId: string) {
    ...
    const recentPurchases = recentAnalytics.reduce(
      (sum, a) => sum + a.purchases,
      0,
    );
    const recentRevenue = recentAnalytics.reduce(
      (sum, a) => sum + Number(a.revenue),
      0,
    );
  ...
  private calculateTopIndustries(purchases: any[]): any[] {
    ...
    purchases.forEach((p) => {
      if (p.companyProfile?.industry) {
        industries[p.companyProfile.industry] =
          (industries[p.companyProfile.industry] || 0) + 1;
      }
    });
