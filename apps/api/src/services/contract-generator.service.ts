...
export class ContractGeneratorService {
  ...
  async generateContractPDF(contract: any): Promise<string> {
    ...
    doc.moveDown(0.5);
    doc.fontSize(11).text(`Hiring Mode: ${this.formatHiringMode(contract.hiringMode)}`);
    doc.moveDown();
    ...
    if (contract.hiringMode === 'full_time') {
      doc.text(`Annual CTC: ${contract.currency} ${contract.ctc?.toLocaleString()}`);
      doc.text(`Placement Fee: ${contract.currency} ${contract.placementFee?.toLocaleString()}`);
    } else if (contract.hiringMode === 'internship') {
      doc.text(`Monthly Stipend: ${contract.currency} ${contract.stipendAmount?.toLocaleString()}`);
      doc.text(`Duration: ${contract.durationMonths} months`);
    } else if (contract.hiringMode === 'hourly_contract') {
      doc.text(`Hourly Rate: ${contract.currency} ${contract.hourlyRate?.toLocaleString()}`);
      doc.text(`Estimated Hours: ${contract.estimatedHours}`);
    } else if (contract.hiringMode === 'project_contract') {
      doc.text(`Total Amount: ${contract.currency} ${contract.totalAmount?.toLocaleString()}`);
    }
    doc.moveDown();
    ...
    doc.text('All work product, inventions, and intellectual property created during the term of this contract shall be owned by the party specified above.');
    doc.moveDown();
    ...
    if (contract.hiringMode === 'project_contract' && contract.milestones) {
      ...
      milestones.forEach((milestone: any, index: number) => {
        doc.text(`${index + 1}. ${milestone.title} - ${contract.currency} ${milestone.amount.toLocaleString()}`);
  ...
  async generateSignedContractPDF(contract: any): Promise<string> {
    ...
    doc.text(`Digital Signature: ${contract.companySignature?.substring(0, 50)}...`);
    doc.moveDown(2);
    ...
    doc.text(`Digital Signature: ${contract.engineerSignature?.substring(0, 50)}...`);
   ...
   */
  async generateSignedAmendmentPDF(contract: any, amendment: any): Promise<string> {
    const doc = new PDFDocument({ margin: 50 });
    const chunks: Buffer[] = [];
