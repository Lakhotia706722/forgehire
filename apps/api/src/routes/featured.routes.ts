...
export async function featuredRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/engineers", async (request, reply) => {
    ...
    const engineersWithRatings = await withDbFallback(async () => {
    ...
    return Promise.all(
      engineers.map(async (eng) => {
        ...
        const rating =
          eng.neuronScore >= 700 ? 4.8 : eng.neuronScore >= 600 ? 4.5 : 4.2;
        const reviewCount = contracts.length;

        return {
          id: eng.id,
          name: eng.fullName,
          headline: eng.headline || eng.bio?.substring(0, 100) || "",
    ...
    await getRedisClient().setex(cacheKey, 600, JSON.stringify(engineersWithRatings));
  ...
  fastify.get("/products", async (request, reply) => {
    ...
    await getRedisClient().setex(cacheKey, 600, JSON.stringify(result));
