...
export async function engineerProfileRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/profile/basic-info', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const body = engineerBasicInfoSchema.parse(request.body);
    const profile = await profileService.updateBasicInfo(request.user.userId, body);
    return successResponse(profile);
  });

  // Add skill (Step 2)
  fastify.post('/profile/skills', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
  ...
  // Update skill
  fastify.put('/profile/skills/:skillId', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
  ...
  // Delete skill
  fastify.delete('/profile/skills/:skillId', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const { skillId } = request.params;
  ...
  fastify.post('/profile/experiences', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const body = engineerExperienceSchema.parse(request.body);
    const experience = await profileService.addExperience(request.user.userId, body);
    return successResponse(experience);
  });

  // Update experience
  fastify.put('/profile/experiences/:experienceId', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const { experienceId } = request.params;
    const body = engineerExperienceSchema.partial().parse(request.body);
    const experience = await profileService.updateExperience(experienceId, body);
    return successResponse(experience);
  });

  // Delete experience
  fastify.delete('/profile/experiences/:experienceId', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const { experienceId } = request.params;
  ...
  // Add project (Step 4)
  fastify.post('/profile/projects', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const body = engineerProjectSchema.parse(request.body);
    const project = await profileService.addProject(request.user.userId, body);
    return successResponse(project);
  });

  // Update project
  fastify.put('/profile/projects/:projectId', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
  ...
  // Delete project
  fastify.delete('/profile/projects/:projectId', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const { projectId } = request.params;
  ...
  fastify.post('/profile/pricing', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const body = engineerPricingSchema.parse(request.body);
    const profile = await profileService.updatePricing(request.user.userId, body);
    return successResponse(profile);
  });

  // Update payment (Step 6)
  fastify.post('/profile/payment', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const body = engineerPaymentSchema.parse(request.body);
    const profile = await profileService.updatePayment(request.user.userId, body);
    return successResponse(profile);
  });

  // Update availability
  fastify.post('/profile/availability', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const body = engineerAvailabilitySchema.parse(request.body);
    const profile = await profileService.updateAvailability(request.user.userId, body);
    return successResponse(profile);
  });

  // Get profile completeness
  fastify.get('/profile/completeness', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const profile = await profileService.getOrCreateProfile(request.user.userId);
    const completeness = await completenessService.calculateCompleteness(profile.id);
    return successResponse(completeness);
  });

  // Get builder progress
  fastify.get('/profile/builder-progress', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const profile = await profileService.getOrCreateProfile(request.user.userId);
  ...
  // Get AI suggestions
  fastify.get('/profile/ai-suggestions', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
    const profile = await profileService.getFullProfile(request.user.userId);
  ...
  // Upload screenshot
  fastify.post('/profile/upload-screenshot', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
  ...
  fastify.post('/profile/presigned-url', {
    preHandler: [authenticate, requireEngineer]
  }, async (request: any, _reply) => {
