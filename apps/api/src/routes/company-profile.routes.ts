...
export async function companyProfileRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/profile/hiring', {
    preHandler: [authenticate, requireCompany]
  }, async (request: any, _reply) => {
    const body = companyHiringSchema.parse(request.body);
    const profile = await profileService.updateHiringStatus(request.user.userId, body);
    return successResponse(profile);
  });

  // Calculate trust score
  fastify.post('/profile/calculate-trust-score', {
    preHandler: [authenticate, requireCompany]
  }, async (request: any, _reply) => {
  ...
  // Upload logo
  fastify.post('/profile/upload-logo', {
    preHandler: [authenticate, requireCompany]
  }, async (request: any, _reply) => {
