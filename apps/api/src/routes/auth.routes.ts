...
import { AuthenticationError, successResponse } from "@neuronhire/shared";
import {
  signupSchema,
  otpVerifySchema,
  refreshTokenSchema,
...
export async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // Exchange Clerk session for Forge API tokens (web app after Clerk sign-in)
  fastify.post("/clerk-session", async (request, _reply) => {
    const authHeader = request.headers.authorization;
    if (!authHeader?.startsWith("Bearer ")) {
      throw new AuthenticationError("Missing Clerk session token");
    }
    const clerkToken = authHeader.slice(7);
    const tokens = await authService.exchangeClerkSession(clerkToken);
    return successResponse(tokens);
  });
