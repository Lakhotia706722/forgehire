...
export async function statsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/platform', async (_request, reply) => {
    ...
    const [
      ...
      prisma.engineerProfile.count({ where: { neuronTier: { not: 'conditional' } } }),
      prisma.engineerProfile.count({ where: { availabilityStatus: 'available_now' } }),
  ...
  fastify.get('/admin', {
    ...
    const [
      ...
      prisma.assessment.count({ where: { tier: { in: ['elite', 'professional', 'verified'] } } }),
      ...
      prisma.contractDispute.count({ where: { status: { not: 'resolved' } } }),
    ...
    const stats = {
      ...
      gmvThisWeek,
      gmvThisMonth,
      assessmentPassRate: assessmentsTotal > 0 ? (assessmentsPassed / assessmentsTotal) * 100 : 0,
      platformFeeToday,
      platformFeeThisWeek,
      platformFeeThisMonth,
      pendingDisputes,
  ...
  fastify.get('/admin/revenue', {
    ...
    const revenueByMonth: Record<string, { contracts: number; bounties: number; marketplace: number; total: number }> = {};

    payments.forEach(p => {
      const d = p.createdAt;
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
      if (!revenueByMonth[key]) revenueByMonth[key] = { contracts: 0, bounties: 0, marketplace: 0, total: 0 };
      const amount = Number(p.amount);
      revenueByMonth[key].total += amount;
      if (p.type === 'milestone_release') revenueByMonth[key].contracts += amount;
      else if (p.type === 'payout') revenueByMonth[key].bounties += amount;
      else if (p.type === 'escrow_deposit') revenueByMonth[key].marketplace += amount;
    });
  ...
  fastify.get('/admin/activity', {
    ...
    const [recentSignups, recentAssessments, recentPayments, recentDisputes] = await Promise.all([
    ...
    recentSignups.forEach(u => activities.push({
      id: `signup-${u.id}`,
      type: 'signup',
    ...
    recentDisputes.forEach(d => activities.push({
      id: `dispute-${d.id}`,
      type: 'dispute_raised',
