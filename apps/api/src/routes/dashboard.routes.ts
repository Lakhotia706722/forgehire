...
export async function dashboardRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/engineer', {
    ...
    const [
      ...
      prisma.taskParticipation.count({
        where: { engineerProfileId: profile.id, approved: false, rejected: false },
      }),
    ...
    const revenueTrend = marketplaceRevenueLastMonth > 0
      ? ((marketplaceRevenueThisMonth - marketplaceRevenueLastMonth) / marketplaceRevenueLastMonth) * 100
      : 0;

    const stats = {
      activeContracts: { count: activeContracts, trend: 0 },
      pendingProposals: { count: pendingProposals, trend: 0 },
      marketplaceRevenue: { amount: marketplaceRevenueThisMonth, trend: Math.round(revenueTrend) },
  ...
  fastify.get('/engineer/recommended-bounties', {
    preHandler: [authenticate],
  }, async (request, reply) => {
    const userId = (request as AuthenticatedRequest).user!.id;
    ...
    const tasks = await prisma.task.findMany({
      where: {
        ...
        ...(skillNames.length > 0 ? { techRequirements: { hasSome: skillNames } } : {}),
      },
    ...
    const result = tasks.map(t => {
      const daysLeft = t.deadline
        ...
        : null;
      const matchingSkills = t.techRequirements.filter(req => skillNames.includes(req));
      const matchPercentage = t.techRequirements.length > 0
        ? Math.round((matchingSkills.length / t.techRequirements.length) * 100)
        : 0;
  ...
  fastify.get('/company', {
    ...
    const [activeTasksPosted, totalEngineersHired, totalSpendThisMonth, openDisputes] = await Promise.all([
      prisma.task.count({
        where: { companyProfileId: profile.id, status: { in: ['open', 'in_progress'] } },
      }),
      prisma.contract.count({
        where: { companyProfileId: profile.id, status: 'completed' },
      }),
      prisma.payment.aggregate({
        where: { userId, status: 'completed', createdAt: { gte: thisMonthStart } },
        _sum: { amount: true },
      }).then(r => Number(r._sum.amount || 0)),
      prisma.contractDispute.count({
        where: { contract: { companyProfileId: profile.id }, status: { not: 'resolved' } },
      }),
    ]);

    const stats = { activeTasksPosted, totalEngineersHired, totalSpendThisMonth, openDisputes };
    await redis.setex(cacheKey, 60, JSON.stringify(stats));
    return reply.send(stats);
  });

  // Company pending submissions
  fastify.get('/company/pending-submissions', {
    preHandler: [authenticate],
  }, async (request, reply) => {
    const userId = (request as AuthenticatedRequest).user!.id;
    const { limit = 10 } = request.query as { limit?: number };
