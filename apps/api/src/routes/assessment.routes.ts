...
const proctoringEventSchema = z.object({
  type: z.enum(['tab_switch', 'focus_loss', 'paste_attempt', 'copy_attempt', 'right_click']),
  timestamp: z.string(),
...
export async function assessmentRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const result = await assessmentService.generateAssessment(profile.id, user.id);
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return successResponse({ message: 'Assessment submitted successfully. Evaluation in progress.' });
    }
  );
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (assessment.status !== 'evaluated') {
        return reply.code(202).send({ success: false, error: 'Assessment evaluation in progress' });
      }
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const events = JSON.parse((assessment.proctoringEvents as string) || '[]');
      ...
      if (body.type === 'paste_attempt') updateData.pasteAttempts = { increment: 1 };
